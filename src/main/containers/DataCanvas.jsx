  ...
  if (!data) {
    return (
      <EmptyState>
        ...
        {!serverOn && <Button enabled onClick={turnServerOn}> Turn HTTP request listening on </Button> }
        {!serverOn && <p>Port: <input id='portNumInput' defaultValue={listeningPort} type="number" min="0" max="8999"/></p>}
        <p>{(serverOn ? `or post JSON to http://localhost:${listeningPort}/` : 'to post JSON directly to Interspect')}</p>
        <br/>
        <Nodatasvg />
      </EmptyState>
    );
